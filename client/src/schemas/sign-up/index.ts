...
export const signUpBaseSchema = z.object({
  fullName: z
    .string()
    .min(5, { message: "Must be at least 5 character(s)" })
    .refine(
      (value) => {
        const names = value.trim().split(/\s+/);
        return names.length >= 2 && names[1].length >= 2;
...
export const signUpSchema = signUpBaseSchema.refine((data) => data.password === data.passwordConfirm, {
...
export function getSignUpFieldError<T extends SignUpFields, K extends SignUpSchema[T]>(property: T, value: K) {
  const { error } = signUpBaseSchema.shape[property].safeParse(value);
  if (!error) return;
